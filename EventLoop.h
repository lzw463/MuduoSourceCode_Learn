...
//事件循环类 主要包含了两个大模块 Channel POller(epoll的封装)
...
private:
    ...
    std::atomic_bool quit_; //标示退出loop循环
    const pid_t threadId_; //记录当前loop所在线程的线程id
    Timestamp pollReturnTime_; //poller返回发生事件的channel的时间点
    std::unique_ptr<Poller> poller_;

    int wakeupFd_; //主要作用：当mainloop获取一个新用户的channel，通过轮询算法选择一个subreactor，通过这个变量唤醒subreactor
